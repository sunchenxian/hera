...
/** Big-endian 162-bit hash suitable for keeping an Ethereum address. */
...
/** Big-endian 12 bytes hash suitable for keeping an asset */
typedef struct evmc_asset
{
    uint8_t bytes[12];
} evmc_asset;
 ...
 * custom instruction
 */
typedef struct evmc_result (*evmc_create_asset_fn)(
        struct evmc_context* context,
        uint32_t assetType,
        uint32_t assetIndex,
        const evmc_uint256be* amount);

typedef struct evmc_result (*evmc_mint_asset_fn)(
        struct evmc_context* context,
        uint32_t assetIndex,
        const evmc_uint256be* amount);

typedef void (*evmc_transfer_fn)(struct evmc_context* context);

typedef struct evmc_result (*evmc_deploy_contract_fn)(
        struct evmc_context* context,
        uint16_t category,
        const uint8_t* template_name_ptr,
...
struct evmc_host_interface
{
    ...
    evmc_create_asset_fn create_asset;

    evmc_mint_asset_fn mint_asset;

    evmc_transfer_fn transfer;

    evmc_deploy_contract_fn deploy_contract;
};
